...
cocte   DB ?            ; DB stays for Define Byte.
...
CALL    pthis           ; Algo de cosmetica...
DB      10, 13, 0       ; ...un salto de linea
PRINTN  'Ingrese el numero de la operacion a realizar:'
CALL    pthis           ; Algo de cosmetica...
DB      10, 13, 0       ; ...un salto de linea
PRINTN  '1) Suma'
PRINTN  '2) Resta'
...
CALL    pthis           ; LLamado a libreria (procedimiento de emu8086.inc)
DB      13, 10, 'El total de la suma es ', 0
...
CALL    pthis           ; LLamado a libreria (procedimiento de emu8086.inc)
DB      13, 10, 'El total de la resta es ', 0
...
DB      13, 10, 'El total de la multiplicacion es ', 0          ; ...esta cadena
...
; Se dividira AX entre BL y el cocte quedara en AL y el resto en AH.
MOV     cocte, al    ; segun lo explicado: en "cocte" el cocte.
...
; cocte y en AH el resto), el print_num no funcionara. Asi que hay que operar.

MOV     dl, cocte       ; copia el valor "real" de cocte
...
PRINT   'El total de la division es '
...
; ************ A PARTIR DE AQUI, PARA EL cocte, SE REPITE EL PROCESO ANTERIOR 
